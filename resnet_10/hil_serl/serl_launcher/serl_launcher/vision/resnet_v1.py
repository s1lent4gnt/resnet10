...
class ResNetBlock(nn.Module):
    ...
    def __call__(
        ...
        if residual.shape != y.shape:
            ...
            print("JAX after conj proj skip :", residual.mean().item())
...
class ResNetEncoder(nn.Module):
    ...
    def __call__(
        ...
        # print("JAX before resize : ", observations.mean().item())
        if observations.shape[-3:-1] != self.image_size:
            observations = resize(observations, self.image_size)

        # print("JAX after resize : ", observations.mean().item())
        ...
        print("JAX After conv:", x.mean().item())

        x = norm(name="norm_init")(x)
        print("JAX After group norm:", x.mean().item())
        x = act(x)
        print("JAX After activation:", x.mean().item())
        x = nn.max_pool(x, (3, 3), strides=(2, 2), padding="SAME")
        # x = nn.avg_pool(x, (3, 3), strides=(2, 2), padding="SAME")
        print("JAX After max_pool :", x.mean().item())
        print("JAX After embedder:", x.shape)
        for i, block_size in enumerate(self.stage_sizes):
            for j in range(block_size):
                ...
                print(f"JAX After block {i}:", x.mean().item())
                if self.use_film:
                    assert (
